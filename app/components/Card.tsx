...
const Card: React.FC<CardProps> = ({ index, activeIndex, setActiveIndex, title, description }) => {
  ...
  return (
    <div className="flex flex-col items-center p-2 cursor-pointer" onClick={() => setActiveIndex(index)}>
      <div className={`bg-white flex flex-col border border-2 p-4 md:p-6 transition-transform duration-300 ease-in-out ${activeIndex === index ? 'border-black' : 'border-gray-200 hover:border-gray-400'}`}>
